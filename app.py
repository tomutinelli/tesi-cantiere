...
<style>
    ...
    /* Stile personalizzato per i pulsanti nativi in Verde Pastello */
...
with st.expander("Note metodologiche e specifiche di utilizzo"):
    ...
    Questo strumento calcola l'impronta di carbonio (kg di CO₂e) in fase di progetto incrociando i dati giornalieri con il database LCI di riferimento. 
    Il dataset finale elaborato è strutturato in 4 colonne: <code>Data</code> (AAAA-MM-GG), <code>Parametro</code>, <code>Elemento</code> e <code>Quantita</code>.
    </p>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    <div style='background-color: #f4f7f3; border: 1.5px solid #d5ddd1; border-radius: 8px; padding: 20px; margin-bottom: 20px;'>
        ...
        <ol style='color: #4b5563; font-size: 0.9rem; line-height: 1.6; margin-bottom: 0; padding-left: 20px;'>
            <li><b>Prepara la documentazione di cantiere:</b> Raccogli i file di progetto (es. computo metrico professionale, cronoprogramma giornaliero, log trasporti).</li>
            <li><b>Carica tutti i file insieme:</b> Seleziona o trascina contemporaneamente tutti i documenti nel riquadro sottostante.</li>
            <li><b>Avvia l'analisi semantica:</b> Clicca su <i>"Elabora e Normalizza con IA"</i> per estrarre e unificare i dati in una serie temporale coerente.</li>
            <li><b>Mappatura automatica LCI:</b> Il motore Python interno mapperà le voci sulle categorie ufficiali del file <code>LCI.xlsx</code>.</li>
            <li><b>Analizza ed esporta:</b> Controlla l'anteprima, scarica i file elaborati e visualizza i grafici interattivi.</li>
        </ol>
    </div>
    """, unsafe_allow_html=True)
    
    st.markdown("<p style='color: #6b7280; font-size: 0.9rem; margin-bottom: 20px;'>Carica tutti i file di progetto insieme (CSV, Excel, PDF o TXT). L'IA unificherà le informazioni e il motore Python le mapperà sul database LCI.</p>", unsafe_allow_html=True)
    
    files_caricati = st.file_uploader(
        "Carica Documenti di Cantiere (puoi selezionare più file insieme)", 
    ...
    if st.button("Elabora e Normalizza con IA", key="btn_ia"):
        ...
        elif not files_caricati:
            st.warning("Carica almeno un file di progetto (es. computo o cronoprogramma) per procedere.")
        else:
            try:
                client = genai.Client(api_key=api_key)
                with st.spinner("L'intelligenza artificiale sta analizzando e unificando tutti i documenti..."):
                    contents = []
                    for file_obj in files_caricati:
                    ...
                    prompt_sistema = """
                    Sei un ingegnere edile e analista LCA. Analizza TUTTI i documenti di cantiere forniti in allegato (computi metrici, cronoprogrammi giornalieri, log trasporti) e fondili in un'unica tabella CSV consolidata con queste esatte 4 intestazioni di colonna:
                    Data,Parametro,Elemento,Quantita
                    
                    - Data: Formato AAAA-MM-GG. Se il computo non ha date, distribuisci le quantità lungo le date del cronoprogramma o del periodo di cantiere.
                    - Parametro: Scegli tassativamente tra: Materiali, Rifiuti, Energia, Acqua, Trasporti, Macchinari.
                    - Elemento: Riporta la descrizione dell'elemento o materiale.
                    ...
                    st.success("Tutti i documenti sono stati elaborati e unificati con successo dall'IA!")
            except Exception as e:
                st.error(f"Errore durante l'elaborazione con l'IA: {e}")

with tab2:
    st.markdown("<p style='color: #6b7280; font-size: 0.9rem; margin-bottom: 20px;'>Importa il file in formato CSV contenente la serie temporale dei consumi già formattata.</p>", unsafe_allow_html=True)
...
if 'df_cantiere' in st.session_state:
    ...
    try:
        ...
        # Normalizzazione robusta per il merge senza duplicati
        ...
        if df_filtrato.empty:
            ...
            # Pulsanti di Download Nativi di Streamlit
